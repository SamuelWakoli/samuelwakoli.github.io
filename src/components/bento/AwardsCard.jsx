            ...
            <div className="space-y-6">
                {profile.awards.map((award, index) => (
                    <div key={index} className="group">
                        <h4 className="text-lg font-medium text-white group-hover:text-yellow-400 transition-colors">{award.name}</h4>
                        <div className="flex justify-between items-center mt-1 mb-2">
                            <span className="text-slate-500 text-xs">{award.issuer}</span>
                            <span className="text-slate-600 text-xs font-mono">{award.year}</span>
                        </div>
                        <p className="text-slate-400 text-sm leading-relaxed">{award.description}</p>
                        {award.link && (
                            <a href={award.link} target="_blank" className="inline-flex items-center gap-1 text-yellow-500/80 text-xs mt-3 hover:text-yellow-400 transition-colors">
